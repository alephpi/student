...
sauve :-
	qsave_program('marienb.exe',[goal=game,autoload=true]).
...
self_play(State, History) :-			% 'History' is the list of visited states
    gameOver(State, Result), 
    !,      % someone has won; 'Result' indicates who won
    write('.'),
    analyse(History, Result).	% analyses the ended game as a list of states ('History')

self_play(State, History) :-
    move(State, NState1), % move instead of ask human player to move
    move(NState1, NState),
    self_play(NState, [NState1, State|History]).
...
learn(N) :-
    ...
    self_play([1,3,5,7], []),
    fail.
	% N = N,	% dummy instruction to avoid warning
	% true.
