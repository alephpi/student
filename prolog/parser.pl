...
% finger_phrase
hp(hui_finger_phrase(HF, number(NP))) --> hf(HF), hnp(NP).
% axp([SF, XP]) --> sf(SF), xp(XP).
xp(xian_finger_phrase(XF, number(NP))) --> xf(XF), xnp(NP).
rxp(xian_finger_phrase(number(N))) --> [N],{in(N, l_XIAN)}.
